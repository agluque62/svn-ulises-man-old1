...
namespace RcsHfSimulator.HfSnmp
{
    ...
    delegate bool SnmpGetAction(ISnmpContext context);
    delegate bool RespondtoGetAction(string oid);
    ...
    /// 
    /// </summary>
    class SnmpGetMessageHandler : IMessageHandler
    {
        public event SnmpGetAction Handled = null;

        public void Handle(ISnmpContext context, ObjectStore store)
        {
            try
            {
                if (Handled != null && Handled(context) == false)
                    return;

                getHandler.Handle(context, store);
            }
            catch (Exception) { }
            finally
            {
        ...
        GetMessageHandler getHandler = new GetMessageHandler();
    }
	...
	static class SnmpAgent
	{
        ...
        public static event RespondtoGetAction RespondToGet = null;
        ...
        /// </summary>
        /// <param name="ip"></param>
		public static void Init(string ip, int port=161, int traps = 162)
		{
			Init(ip, null, port, traps);
		}
        ...
        /// 
        /// </summary>
        static SnmpGetMessageHandler getHandler = new SnmpGetMessageHandler();
        ...
        public static void Init(string ip, string trapMcastIp, int port, int trap)
		{
            ...
            getHandler.Handled += getHandler_PduGetReceived;
		}
        ...
        private static bool getHandler_PduGetReceived(ISnmpContext context)
        {
            if (!_closed)
            {
                var pdu = context.Request.Scope.Pdu;
                {
                    ...
                    foreach (var v in pdu.Variables)
                    {
                        if (RespondToGet != null && RespondToGet(v.Id.ToString())==false)
                            result = false;
                    }
                    return result;
                }
